...
//
template <const size_t N, const uint32_t MOD[(N + 31) / 32], const uint32_t &M0, const uint32_t RR[(N + 31) / 32], const uint32_t ONE[(N + 31) / 32], const uint32_t MODx[(N + 31) / 32] = MOD>
...
private:
    ...
    static inline void mul_n(uint32_t *acc, const uint32_t *a, uint32_t bi, size_t n = (N + 31) / 32)
    {
        for (size_t j = 0; j < n; j += 2)
            asm("mul.lo.u32 %0, %2, %3; mul.hi.u32 %1, %2, %3;" : "=r"(acc[j]), "=r"(acc[j + 1]) : "r"(a[j]), "r"(bi));
    }

    static inline void cmad_n(uint32_t *acc, const uint32_t *a, uint32_t bi, size_t n = (N + 31) / 32)
    {
    ...
    static inline void cadd_n(uint32_t *acc, const uint32_t *a, size_t n = (N + 31) / 32)
    {
    ...
    private:
        static inline void mad_row(uint32_t *odd, uint32_t *even, const uint32_t *a, uint32_t bi, size_t n = (N + 31) / 32)
        {
...
#pragma unroll
            ...
            asm("mul.lo.u32 %0, %2, %3; mul.hi.u32 %1, %2, %3;" : "=r"(even[2 * n - 4]), "=r"(even[2 * n - 3]) : "r"(a[n - 1]), "r"(a[n - 3]));
            asm("mad.lo.cc.u32 %0, %2, %3, %0; madc.hi.cc.u32 %1, %2, %3, %1;" : "+r"(odd[2 * n - 6]), "+r"(odd[2 * n - 5]) : "r"(a[n - 2]), "r"(a[n - 3]));
            asm("addc.u32 %0, %0, 0;" : "+r"(even[2 * n - 3]));

            asm("mul.lo.u32 %0, %2, %3; mul.hi.u32 %1, %2, %3;" : "=r"(odd[2 * n - 4]), "=r"(odd[2 * n - 3]) : "r"(a[n - 1]), "r"(a[n - 2]));
            ...
            i = 0;
            asm("mad.lo.cc.u32 %0, %2, %2, %0; madc.hi.cc.u32 %1, %2, %2, %1;" : "+r"(even[2 * i]), "+r"(even[2 * i + 1]) : "r"(a[i]));
            for (++i; i < n; i++)
                asm("madc.lo.cc.u32 %0, %2, %2, %0; madc.hi.cc.u32 %1, %2, %2, %1;" : "+r"(even[2 * i]), "+r"(even[2 * i + 1]) : "r"(a[i]));
        }
    };
...
public:
    inline void print() { 
        mont_t m = (*this), one_raw; one_raw.even[0] = 1;
        for (size_t i = 1; i < n; i++) one_raw.even[i] = 0;
        m *= one_raw;
        for (size_t i = n; i > 0; i--) printf("%08x", m.even[i - 1]); printf("\n"); 
    }
