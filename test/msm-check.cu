...
int main(int argc, char *argv[])
{
    ...
    // for (size_t idx = 20; idx <= 28; idx++) {
        // size_t n = 1 << idx;

        xpu::vector<libff::alt_bn128_G1> dev_points(n, xpu::mem_policy::device_only), dev_res(1, xpu::mem_policy::cross_platform);
        ...
        (fix_base_multi_scalar_multiplication_g1<alt_bn128::fr_t, alt_bn128::g1_t>)<<<14, 510>>>((alt_bn128::g1_t*)dev_points.p(), (alt_bn128::fr_t*)dev_scalars.p(), n);
        CUDA_DEBUG;
        timer.stop();
        ...
        CUDA_DEBUG;
        timer.start();
        light_msmg1<alt_bn128::fr_t, alt_bn128::g1_t>(n, (alt_bn128::fr_t*)dev_scalars.p(), (alt_bn128::g1_t*)dev_points.p(), (alt_bn128::g1_t*)dev_res.p(), win_siz, win_cnt, bucket_cnt);
        CUDA_DEBUG;
        timer.stop();
        dev_res.load();

        xpu::vector<libff::alt_bn128_G1> points(n);
        points.load(dev_points.p(), n);
